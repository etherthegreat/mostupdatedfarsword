...
2. Determine which script and status string to use based on SOURCE:
...
3. Check whether the art PNG exists:
...
4. Edit the script, then run the appropriate builder:
   - Flavordoc tasks: `python3 scripts/build_flavordoc.py`
   - Event tasks: `python3 scripts/build_event_masterdoc.py`

5. Commit and push all changed files (the builder script + the regenerated `.xlsx`) to `claude/farsword-recent-push-JL6OS`. Commit message: `Close ethertask: <task id>`.

6. Report back: what was closed, which status it moved to, and whether art was marked done or is still pending.
